...
class EloStatsController @Inject()(
  ...
  def viewStats(userId: Long): Action[AnyContent] = silhouette.SecuredAction.async { implicit request =>
    for {
      ...
      Ok(views.html.eloStats(request.identity, userId, userAliasOpt.getOrElse("Unknown"), currentElos, eloLogs))
    }
  }
}