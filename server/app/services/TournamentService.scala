...
/** Service interface for Tournament operations including tournaments,
...
trait TournamentService {
  ...
  /** Retrieves tournaments by status.
    ...
    * @return
    *   List of tournaments with the specified status
    */
  ...
  /** Retrieves tournaments that are open for registration.
    *
    * @return
    *   List of tournaments open for registration
    */
  ...
  /** Updates a tournament.
    ...
    * @return
    *   The updated tournament if found, None otherwise
    */
  ...
  /** Closes registration for tournaments whose registration period has ended.
    *
    * @return
    *   List of tournaments whose registration was closed
    */
  ...
  /** Retrieves all registrations for a tournament.
    ...
    */
  def getTournamentRegistrations(
      tournamentId: Long
  ): Future[List[TournamentRegistration]]
  ...
  /** Retrieves all registrations with user details for a tournament.
    ...
    */
  def getTournamentRegistrationsWithUsers(
      tournamentId: Long
  ): Future[List[(TournamentRegistration, User)]]
  ...
  /** Retrieves active registrations for a tournament.
    ...
    *   The tournament ID
    * @return
    *   List of active registrations
    */
  def getActiveRegistrations(
      tournamentId: Long
  ): Future[List[TournamentRegistration]]
  ...
  /** Retrieves a tournament match by its ID and tournament ID. If the match
    * doesn't exist locally, fetches it from Challonge API and creates it.
    *
    * @param challongeMatchID
    *   The match ID
    * @param tournamentId
    *   The tournament ID
    * @return
    *   The tournament match if found, None otherwise
    */
  def getMatch(
      tournamentId: Long,
  ...
  /** Retrieves all matches for a specific user.
    ...
    *   The user ID
    * @return
    *   List of tournament matches where the user is participating
    */
  ...
  /** Updates the status of a tournament match.
    ...
    *   The new status
    * @return
    *   The updated tournament match if found, None otherwise
    */
  ...
  /** Starts a tournament match by changing its status to InProgress.
    ...
    *   The match ID
    * @return
    *   The updated tournament match if found and was in Pending status, None
    *   otherwise
    */
  ...
  /** Completes a tournament match by changing its status to Completed.
    ...
    *   The match ID
    * @return
    *   The updated tournament match if found and was in InProgress status, None
    *   otherwise
    */
  ...
  /** Retrieves matches by status.
    ...
    *   The match status
    * @return
    *   List of tournament matches with the specified status
    */
  ...
  /** Retrieves pending matches for a specific tournament.
    ...
    *   The tournament ID
    * @return
    *   List of pending tournament matches
    */
  ...
  /** Retrieves active (InProgress) matches for a specific tournament.
    ...
    *   The tournament ID
    * @return
    *   List of active tournament matches
    */
...
/** Implementation of TournamentService.
  ...
  *   The tournament repository implementation.
  * @param tournamentMatchRepository
  *   The tournament match repository implementation.
  * @param tournamentRegistrationRepository
  *   The tournament registration repository implementation.
  * @param tournamentChallongeService
  *   The Challonge service for API interactions.
  * @param tournamentChallongeParticipantRepository
  *   The repository for Challonge participant mappings.
  *
  * @param uploadedFileRepository
  *   The uploaded file repository for persisting file records.
  * @param ec
...
class TournamentServiceImpl @Inject() (
  ...
  /** Retrieves tournaments by status.
    */
  override def getTournamentsByStatus(
  ...
  /** Updates tournament status.
    */
  override def updateTournamentStatus(
      id: Long,
      newStatus: TournamentStatus
  ): Future[Option[Tournament]] = {
  ...
  /** Sets the Challonge tournament ID for a tournament.
    */
  override def setChallongeTournamentId(
      id: Long,
      challongeTournamentId: Long
  ): Future[Option[Tournament]] = {
  ...
  override def closeExpiredRegistrations(): Future[List[Tournament]] = {
    for {
      ...
      updatedTournaments <- Future.sequence(expiredTournaments.map {
        tournament =>
          tournamentRepository.updateStatus(
            tournament.id,
      ...
      result <- tournamentOpt match {
        ...
        case Some(tournament) =>
          ...
          if (tournament.status != TournamentStatus.RegistrationOpen) {
            Future.successful(
              Left("Registration is not open for this tournament")
            )
            ...
            for {
              isAlreadyRegistered <- tournamentRegistrationRepository
                .isUserRegistered(tournamentId, userId)
              result <-
                if (isAlreadyRegistered) {
                  Future.successful(
                    Left("User is already registered for this tournament")
                  )
                } else {
                  for {
  ...
  /** Withdraws a user's registration from a tournament.
    */
  override def withdrawRegistration(
      tournamentId: Long,
      userId: Long
  ): Future[Boolean] = {
    for {
      registrationOpt <- tournamentRegistrationRepository
      ...
      result <- registrationOpt match {
        ...
        case Some(registration) =>
          tournamentRegistrationRepository
            .updateStatus(registration.id, RegistrationStatus.Withdrawn)
            .map(_.isDefined)
      }
    } yield result
  }
  ...
  /** Retrieves all registrations for a tournament.
    */
  override def getTournamentRegistrations(
      tournamentId: Long
  ): Future[List[TournamentRegistration]] = {
  ...
  /** Retrieves all registrations with user details for a tournament.
    */
  override def getTournamentRegistrationsWithUsers(
      tournamentId: Long
  ): Future[List[(TournamentRegistration, User)]] = {
  ...
  /** Retrieves active registrations for a tournament.
    */
  override def getActiveRegistrations(
      tournamentId: Long
  ): Future[List[TournamentRegistration]] = {
  ...
  /** Fetches a match from Challonge API and creates it locally with user
    * mappings.
    */
  private def fetchMatchFromChallonge(
      challongeMatchID: Long,
      tournamentId: Long
  ): Future[Option[TournamentMatch]] = {
    for {
      ...
      result <- tournamentOpt match {
        case Some(tournament) if tournament.challongeTournamentId.isDefined =>
          ...
          for {
            ...
            challongeMatches <- tournamentChallongeService.getMatches(
              challongeTournamentId
            )

            // Find the specific match
            matchResult <- challongeMatches.find(
              _.id == challongeMatchID
            ) match {
              case Some(challongeMatch) =>
                // Convert Challonge participants to local user IDs
                convertChallongeMatchToTournamentMatch(
                  challongeMatch,
                  tournamentId,
                  challongeTournamentId
                )
  ...
  private def convertChallongeMatchToTournamentMatch(
    ...
    for {
      participantMappings <- tournamentChallongeParticipantRepository
        .findByChallongeTournamentId(challongeTournamentId)
      participantMap = participantMappings
        .map(p => p.challongeParticipantId -> p.userId)
        .toMap

      result <- (challongeMatch.player1Id, challongeMatch.player2Id) match {
        case (Some(player1ChallongeId), Some(player2ChallongeId)) =>
          (
            participantMap.get(player1ChallongeId),
            participantMap.get(player2ChallongeId)
          ) match {
  ...
  /** Retrieves all matches for a specific tournament.
    */
  override def getMatchesForTournament(
      tournamentId: Long
  ): Future[List[TournamentMatch]] = {
    tournamentMatchRepository.findByTournamentId(tournamentId)
  }

  /** Retrieves all matches for a specific user.
    */
  override def getMatchesForUser(
      userId: Long
  ): Future[List[TournamentMatch]] = {
  ...
  /** Updates the status of a tournament match.
    */
  override def updateMatchStatus(
      matchId: Long,
      newStatus: MatchStatus
  ): Future[Option[TournamentMatch]] = {
  ...
  override def startMatch(matchId: Long): Future[Option[TournamentMatch]] = {
    for {
      ...
      result <- matchOpt match {
        case Some(matchTournament)
            if matchTournament.status == MatchStatus.Pending =>
          tournamentMatchRepository.updateStatus(
            matchId,
  ...
  override def completeMatch(matchId: Long): Future[Option[TournamentMatch]] = {
    for {
      ...
      result <- matchOpt match {
        case Some(matchTournament)
            if matchTournament.status == MatchStatus.InProgress =>
  ...
  /** Retrieves matches by status.
    */
  override def getMatchesByStatus(
      status: MatchStatus
  ): Future[List[TournamentMatch]] = {
  ...
  /** Retrieves pending matches for a specific tournament.
    */
  override def getPendingMatches(
      tournamentId: Long
  ): Future[List[TournamentMatch]] = {
    tournamentMatchRepository.findByTournamentIdAndStatus(
      tournamentId,
      MatchStatus.Pending
    )
  }
  ...
  /** Submits match result and updates both local database and Challonge. Also
    * closes upload sessions and registers uploaded files in the database.
    */
  override def submitMatchResult(
    ...
    for {
      ...
      tournamentOpt <- tournamentRepository.findById(
      ...
      result <- tournamentOpt match {
        case Some(
          ...
          for {
            localUpdateResult <- tournamentMatchRepository
              .updateWinnerAndStatus(tournamentMatch, winner)
            _ <-
              if (localUpdateResult) Future.successful(true)
              else Future.failed(new NoSuchElementException("Updated failed"))
            challongeUpdateResult <- updateChallongeMatch(
              challongeTournamentId,
              tournamentMatch,
              winner
            )
            _ <-
              if (challongeUpdateResult) Future.successful(true)
              else
                Future.failed(new NoSuchElementException("Challonge updated"))
          } yield {
            true
          }
  ...
  /** Extracts the relative directory path from the full stored path. This
    * assumes the stored path is within the configured upload directory.
    */
  ...
  private def updateChallongeMatch(
    ...
    for {
      ...
      participantMappings <- tournamentChallongeParticipantRepository
        .findByChallongeTournamentId(challongeTournamentId)
      userToChallongeMap = participantMappings
        .map(p => p.userId -> p.challongeParticipantId)
        .toMap
