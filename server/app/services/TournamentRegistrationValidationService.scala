...
/** Service for validating tournament registration requirements. This service is
  * separate from TournamentService to avoid circular dependencies.
  */
trait TournamentRegistrationValidationService {

  /** Checks if user has enough base replays for a given race. Requires 2
    * replays per matchup (6 total: 2 vs each race including mirror).
    *
    * @param userId
    *   The user ID
    * @param race
    ...
    */
  def hasEnoughReplays(
      userId: Long,
      race: SCRace,
      minReplaysPerMatchup: Int = 2
  ): Future[Boolean]
  ...
  /** Gets the replay counts per matchup for a user's race.
    ...
    */
  def getReplayCountsPerMatchup(
      userId: Long,
      race: SCRace
  ): Future[Map[SCRace, Int]]
...
class TournamentRegistrationValidationServiceImpl @Inject() (
  ...
  override def hasEnoughReplays(
    ...
    for {
      ...
      allRaces.forall(rivalRace =>
        replayCounts.getOrElse(rivalRace, 0) >= minReplaysPerMatchup
      )
    }
  }

  override def getReplayCountsPerMatchup(
      userId: Long,
      race: SCRace
  ): Future[Map[SCRace, Int]] = {
  ...
  override def isUserAbleToRegister(
      userId: Long,
      racePicked: SCRace
  ): Future[Boolean] = {
    for {
      hasReplays <- hasEnoughReplays(
        userId,
        racePicked,
        minReplaysPerMatchup = 2
      )
